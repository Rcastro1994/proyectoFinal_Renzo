...
def descargarReporteUsuarios(request):
    ...
    archivoPdf.setFont('Helvetica-Bold',12)
    archivoPdf.drawString(40,620, 'Fecha creación:')
    archivoPdf.drawString(40,605, 'Cantidad usuarios:')
    archivoPdf.drawString(40,590, 'Nombre que genera:')
    ...
    archivoPdf.drawString(50,538,  'Nombre')
    archivoPdf.drawString(130,538, 'Apellido')
    archivoPdf.drawString(210,538, 'Username')
    archivoPdf.drawString(275,538, 'FechaIngreso')
    archivoPdf.drawString(370,538, 'Celular')
    archivoPdf.drawString(430,538, 'Tareas')
    archivoPdf.drawString(470,538, 'Tipo Usuario')
    ...
    for usuario in usuarioInformacion:
        ...
        archivoPdf.drawString(lista_x[0] + 235, lista_y[1]-15, f'{usuario.datosusuario.fechaIngreso}')
        archivoPdf.drawString(lista_x[0] + 330, lista_y[1]-15, f'{usuario.datosusuario.nroCelular}')
        archivoPdf.drawString(lista_x[0] + 401, lista_y[1]-15, f'{tareasInformacion.objects.filter(usuarioRelacionado=usuario).count()}')
...
def actualizarEstadoTarea(request):
    datos = json.load(request)
    tareaID = datos.get('idTarea')
    actualizarTarea = tareasInformacion.objects.get(id=tareaID)
    actualizarTarea.estadoTarea = "Finalizado"
...
def actualizarInfoUsuario(request):
    ...
    usuario.profesionUsuario = profesionUsuario
    usuario.save()    
    return JsonResponse({
        'resp':'ok'
    })
